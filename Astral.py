...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Astral - " + os.path.basename(self.projectdir))
...
def main():
    ...
    Astral = MainWindow()
    Astral.setStyle(QtWidgets.QStyleFactory.create(Astral.qt_style))
    Astral.setWindowIcon(Astral.icon)
    with open(os.path.join(Astral.theme_dir, Astral.theme_name + '.css')) as file:
        Astral.setStyleSheet(file.read())
    Astral.raise_() #Making the window get focused on OSX
    sys.exit(app.exec_())
