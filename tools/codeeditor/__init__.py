...
class CodeEditor(QtWidgets.QDialog):
	def __init__(self, main, current_file):
		...
		if "objects" in self.current_file:
			...
			print("Error detecting file type, defaulting to Python")
			extension = ""
			self.syntax_mode = "python"
		...
		QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+S"), self, self.save)

	def save(self):
		jscript = "editor.getValue();"
		current_text = self.webkit.page().runJavaScript(jscript, self.finishSave)
				
	def finishSave(self, current_text):
...
if __name__ == "__main__":
	...
	mainWin = CodeEditor("~/astral/Astral.py")
	mainWin.show()
